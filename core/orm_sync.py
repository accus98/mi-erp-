...
class Model(metaclass=MetaModel):
    ...
    def _get_restricted_fields(self):
        ...
        try:
            ...
            self.env.cr.connection.rollback()
            return {}
    ...
    def _filter_authorized_fields(self, operation, fields):
        if self.env.uid == 1: return fields
        
        restricted = self._get_restricted_fields()
        if not restricted: return fields
        
        # Optim: Only fetch groups if we hit a restricted field
        common = set(fields).intersection(restricted.keys())
        if not common: return fields
        
        # Check Groups
        # Get user groups
        user = self.env['res.users'].browse(self.env.uid)
        # Assuming user.groups_id is a Many2many returning recordset, we want ids
        # But to avoid recursion if we are reading res.users?
        # Direct SQL safe for groups?
        # Let's rely on cache or lightweight read.
        # But wait, we are in 'read', calling 'browse' -> 'read'...
        # Risk of recursion if FLS is on res.users!
        
        # Safe way: Direct SQL for user groups
        if not hasattr(self.env, '_user_groups_cache'):
            self.env._user_groups_cache = {}
            
        if self.env.uid not in self.env._user_groups_cache:
            self.env.cr.execute("""
                SELECT res_groups_id FROM res_groups_users_rel WHERE res_users_id = %s
            """, (self.env.uid,))
            self.env._user_groups_cache[self.env.uid] = set(r[0] for r in self.env.cr.fetchall())
            
        user_groups = self.env._user_groups_cache[self.env.uid]
        
        allowed = []
        for f in fields:
            if f not in restricted:
                allowed.append(f)
            else:
                required = restricted[f]
                # If user has ANY of the required groups -> Allowed
    ...
    def _filter_authorized_fields(self, operation, fields):
        ...
        # Safe way: Direct SQL for user groups to avoid recursion
        if not hasattr(self.env, '_user_groups_cache'):
            self.env._user_groups_cache = {}
    ...
    def read(self, fields=None):
        ...
        fields = self._filter_authorized_fields('read', fields)
    ...
    def write(self, vals):
        ...
        allowed_keys = self._filter_authorized_fields('write', keys_to_write)
        if len(allowed_keys) != len(keys_to_write):
             forbidden = set(keys_to_write) - set(allowed_keys)
             raise Exception(f"Security Error: Access Denied to write fields {forbidden} on {self._name}")
        ...
        if m2m_values:
            for field, target_ids in m2m_values.items():
                ...
                # Delta Logic
                # 1. Get existing relations for ALL records in self
                # This is still O(1) query: SELECT col1, col2 FROM rel WHERE col1 IN self.ids
                q_get = f'SELECT "{f_obj.column1}", "{f_obj.column2}" FROM "{f_obj.relation}" WHERE "{f_obj.column1}" = ANY(%s)'
                self.env.cr.execute(q_get, (list(self.ids),))
                existing = self.env.cr.fetchall()
                
                # Map record_id -> set(related_ids)
                existing_map = {}
                ...
                to_insert = []
                to_delete_params = [] # List of (rid, tid) or optimized
                ...
                for rid in self.ids:
                    ...
                    for tid in removing:
                         to_delete_params.append((rid, tid))
                         
                # Execute Checks
                if to_delete_params:
                    # DELETE FROM rel WHERE (col1=r1 AND col2=t1) OR ...
                    # Composite IN is strict in Postgres: WHERE (col1, col2) IN ((r1,t1), ...)
                    # Check Postgres version support? Usually yes in modern PG.
                    # Or simple approach: DELETE WHERE col1=r AND col2 IN (tids) - loop?
                    # Bulk delete is better.
                    
                    # Using mogrify for valid SQL IN clause
                    args_str = ','.join(self.env.cr.mogrify("(%s,%s)", x).decode('utf-8') for x in to_delete_params)
                    self.env.cr.execute(f'DELETE FROM "{f_obj.relation}" WHERE ("{f_obj.column1}", "{f_obj.column2}") IN ({args_str})')

                if to_insert:
                    args_str = ','.join(self.env.cr.mogrify("(%s,%s)", x).decode('utf-8') for x in to_insert)
